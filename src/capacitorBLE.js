...
class CapacitorBLESerial {
    ...
    async connect() {
        try {
            ...
            // Small delay to let ESP32 settle
            await new Promise(r => setTimeout(r, 200));

            // Start notifications (this should trigger CCCD write)
            await this.bleClient.startNotifications(
                this.device.deviceId,
                MICROBLOCKS_SERVICE_UUID,
                MICROBLOCKS_TX_CHAR_UUID,
                (data) => {
                    // Safer extraction in case DataView has offset
        ...
        try {
            for (let i = 0; i < data.length; i += BLE_PACKET_LEN) {
                const chunk = data.slice(
                    i,
                    Math.min(i + BLE_PACKET_LEN, data.length)
                );
